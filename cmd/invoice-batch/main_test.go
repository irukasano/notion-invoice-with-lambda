...
func TestHandleLogsStartAndFinish(t *testing.T) {
	...
	ctx := logger.ContextWithRequestID(context.Background(), "req-batch")
	...
	assertLog(t, logs[0], "INFO", "invoice-batch started", "req-batch")
	assertLog(t, logs[1], "INFO", "invoice-batch finished", "req-batch")
}
...
func TestHandleLogsFailure(t *testing.T) {
	...
	assertLog(t, logs[0], "INFO", "invoice-batch started", "req-batch")
	assertLog(t, logs[1], "ERROR", "invoice-batch failed", "req-batch")
