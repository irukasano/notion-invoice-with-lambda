...
func TestHandleLogsStartAndFinish(t *testing.T) {
	...
	ctx := logger.ContextWithRequestID(context.Background(), "req-cashflow")
	...
	assertLog(t, logs[0], "INFO", "cashflow-update started", "req-cashflow")
	assertLog(t, logs[1], "INFO", "cashflow-update finished", "req-cashflow")
}
...
func TestHandleLogsFailure(t *testing.T) {
	...
	assertLog(t, logs[0], "INFO", "cashflow-update started", "req-cashflow")
	assertLog(t, logs[1], "ERROR", "cashflow-update failed", "req-cashflow")
